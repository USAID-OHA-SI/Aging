...
visits_data <- read_csv("Data/AgingOutVisits.csv", 
                        na = c("", "NA", "NULL"),
                        col_types = list(.default = "c"))
...
exits_data <- read_csv("Data/AgingOut.Exits.csv", 
                       na = c("", "NA", "NULL"),
                       col_types = list(.default = "c"))
...
unique(exits_data$exit_reason) |> sort()
...
his_data <- read_csv("Data/AgingOut.HISList.csv", 
                     na = c("", "NA", "NULL"),
                     col_types = list(.default = "c"))
...
#are the truncated ids still unique?
  ...
  paste("There are", length(unique(clientlist$nupi)) %>% comma, "unique national clients")
...
#are the truncated ids still unique?
  length(unique(visits_data$id)) == orig_unique
  
  paste("There are", length(unique(visits_data$id)) %>% comma, "unique clients with visits vs", comma(orig_unique), "clients on file")
  paste("There are", length(unique(visits_data$nupi)) %>% comma, "unique national clients with visits vs", comma(orig_nupi), "clients on file")
...
#are the truncated ids still unique?
  length(unique(exits_data$id)) == orig_unique
...
# Check HIS for Multiple IMs at a single facility
  his_data %>% count(agency, sort = T)
  his_data %>% count(im, sort = T)
  his_data %>% count(agency, im, sort = T)
...
master_clientlist %>% 
  ...
  mutate(new = date_art_init_fy, # Use `curr` instead of new (I think this makes it clearer)
...
#| label: visits_clean
  ...
  #reorder columns for ease of review
    visits_data <- visits_data %>% 
      relocate(id, nupi, facility, .before = 1) %>% 
