...
viz_collapsed <- viz_collapsed %>% 
  # mutate(status_orig = status_start_yr) %>% 
  group_by(id) %>% 
...
viz_collapsed <- viz_collapsed %>% 
  mutate(status_start_yr = ifelse(filled == TRUE & status_start_yr == "New", NA_character_, status_start_yr)) %>%
  select(-filled)
...
#filter down new so these can be readded to the dataset to act like the year before in the viz
#move new to start for placement on plot (will move manually to mid point)
viz_collapsed_new <- viz_collapsed %>% 
  filter(str_detect(status_start_yr, "New"),
         fiscal_year != 2019) %>% 
  mutate(fiscal_year = fiscal_year - 1,
         status_start_yr = "New",
         # status_orig = NA_character_,
         orig_new = FALSE)

#bind the new data on and replace flagged news with NA
viz_collapsed <- viz_collapsed %>%
  mutate(status_start_yr = str_remove(status_start_yr, " - New")) %>% 
  bind_rows(viz_collapsed_new) %>%
  arrange(id, fiscal_year) %>%
  mutate(status_start_yr = ifelse(orig_new == TRUE, NA_character_, status_start_yr)) %>% 
  select(-orig_new)
