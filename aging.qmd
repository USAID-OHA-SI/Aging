...
```
So at the end of the day, we have `r nrow(master_clientlist) %>%  comma()` unique client x facility pairs.
...
We will want to remove any instances in the data where new initations are occurring after a patient has been to one or more visits. We will filter out any visits that occur before the `data_art_init`.
